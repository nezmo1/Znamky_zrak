        ...
        protected function createComponentNovyTrida()
	{
		...
		$form->addText('zkratka_tridy', 'Zkratka třídy')
			...
			->setRequired('Zadejte název třídy');
                  ...
                  $form->addText('barva_ramu', 'Barva rámu')
                        ->setAttribute('placeholder', 'Barva rámu')
        ...
        function novyTrida($form, $values){
               ...
               $uzivatel="";
                ...
                if ($values->zkratka_tridy==$uzivatel){
                    $flashMessage = $this->flashMessage('Třída s touto zkratkou již existuje!','warning'); 
                  return $flashMessage;
                } 
                else {
         ...
         protected function createComponentNovyPredmet()
	{
		...
		$form->addText('zkratka_predmetu', 'Zkratka předmětu')
			...
			->setRequired('Zadejte název předmětu');
                 ...
                 $form->addText('barva_textu', 'Barva textu')
                        ->setAttribute('placeholder', 'Barva textu')
			
                       
                        ->setDefaultValue('black');
                  
                  
                   $form->addText('barva_pozadi', 'Barva pozadí')
                        ->setAttribute('placeholder', 'Barva pozadí')
            ...
            $existuje =  $this->database->table('predmet')->where('zkratka_predmetu="'.$values->zkratka_predmetu.'"')->fetch();
               $uzivatel="";
                ...
                if ($values->zkratka_predmetu==$uzivatel){
                    $flashMessage = $this->flashMessage('Předmět s touto zkratkou již existuje!','warning'); 
                  return $flashMessage;
                } 
                else {
                    
                    if($this->database->query('INSERT INTO predmet SET zkratka_predmetu= ?',$values->zkratka_predmetu,', barva_textu = "'.$values->barva_textu.'", barva_pozadi = "'.$values->barva_pozadi.'", nazev= ?',$values->nazev,'')){
                     $flashMessage = $this->flashMessage('Předmět byl úspěšně přidán do databáze.','success');    
                    }
                    else {
                     $flashMessage = $this->flashMessage('Chyba databáze na straně serveru, předmět nebyl přidán!','error');    
                    }
