 ...
 */
        ...
        protected function createComponentEditTridaForm()
	{
		$form = new Nette\Application\UI\Form;
		...
		$form->addText('zkratka_tridy', 'Zkratka třídy')
			...
			->setRequired('Zadejte název třídy')
                 ...
                 $form->addText('barva_ramu', 'Barva rámu')
                        ...
                        ->setDefaultValue($nazev_tridy->barva_ramu);
        ...
        function editTrida($form, $values){
             ...
             //   echo $this->database->query('UPDATE trida SET jmeno_tridy= ?',$values->jmeno_tridy,', zkratka_tridy= ?',$values->zkratka_tridy,', barva_ramu = "'.$values->barva_ramu.'" WHERE id_tridy= ?', $values->id_tridy);
                    
                    if($this->database->query('UPDATE trida SET jmeno_tridy="'.$values->jmeno_tridy.'", zkratka_tridy= "'.$values->zkratka_tridy.'", barva_ramu = "'.$values->barva_ramu.'" WHERE id_tridy= '.$values->id_tridy)){
                     $flashMessage = $this->flashMessage('Třída byla úspěšně editována.','success');    
                    }
                    else {
                     $flashMessage = $this->flashMessage('Chyba databáze na straně serveru, skupina nebyla editována!','error');    
                    }
