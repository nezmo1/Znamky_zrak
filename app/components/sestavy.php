...
class Sestavy extends Nette\Application\UI\PresenterComponent
{
    ...
    public function render()
    {
       ...
       $predmets=array();
       $zaci=  $this->database->query('SELECT id_users,CONCAT(prijmeni," ",jmeno) as `cele_jmeno` FROM users where trida !="42" ORDER BY trida');
       $zacis=array(); 
      ...
      // for($i=0;$i<$pocet->pocet;$i++){
           foreach($zacis as $zak => $values) { 
            echo "<table border='1' style='border-collapse: collapse;font-size:13px'>";
            echo "<tr>";
            echo "<td></td>";
           foreach($predmets as $predmet => $value)
{
            echo "<td style='text-align:center; width:35px' >";
            echo $value;
            echo "</td>";  
}
        ...
        echo "<tr>";
        echo "<td style='width:150px' >".$values."</td>";
